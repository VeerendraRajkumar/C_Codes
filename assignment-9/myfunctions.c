...
void addm(int a[][3],int b[][3],int m,int n,int s[][3]) 
{
 int i,j;
...
void prom(int a[][3],int b[][3],int n,int p[][3])
{
 int i,j,k;
...
void transpose(int a[][3],int n ,int t[][3])
{
  int i,j;
...
int checksymmetric(int a[][3],int n)
{
 int i,j;
...
int checkmagicsq(int a[][10],int n)
{
 int i,j,f=1,sc=0,sc2=0,sr=0,sr2=0,d=0,d2=0;

 i=checkduplicate(a,n);
 if(!i)
 { for(i=0;i<n;i++)
   for(j=0;j<n;j++)
    if(a[i][j]>n*n)
     ...
     for(i=0;i<n;i++)
      {
       ...
       for(j=0;j<n;j++)
        { 
          if(i<n-1)
           {
            sc+=a[i][j];sc2+=a[i+1][j];
            sr+=a[j][i];sr2+=a[j][i+1];
           } 
          if(i+j==n-1)
           {
            d2+=a[i][j];
            d+=a[i][i];//This should be done only n times.
           }
        }
       if(sc!=sc2||sr!=sr2||sc!=sr)
        return 0;
      }
      if(d!=d2)
       return 0;
  ...
  { 
   printf("\n\tPlease enter without duplicates.\n");
   return -1;
  }
}

int checkduplicate(int a[][10],int n)
{
  int i,j,i2,j2,c;
